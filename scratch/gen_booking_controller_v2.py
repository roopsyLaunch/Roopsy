...
async function autoSuggestChair(barber, startTime, durationMinutes) {
  ...
  const availableSeats = barber.seats.filter(s => s.isAvailable && s.status !== 'maintenance' && !occupiedSeatIndices.includes(s.index));
...
const createSchema = z.object({
  ...
  staffId: z.string().length(24).optional(),
});

async function create(req, res) {
  const parsed = createSchema.safeParse(req.body);
  if (!parsed.success) return res.status(400).json({ error: parsed.error.flatten() });
  const { barberId, serviceIds, startTime, notes, seatIndex, isHomeService, staffId } = parsed.data;
  ...
  if (!lock.lastErrorObject.updatedExisting && lock.value && lock.value.lockedBy.toString() !== req.user._id.toString()) {
     return res.status(409).json({ error: "This slot is currently being locked by another user." });
  }

  // Also verify actual Bookings overlap to be strictly atomic against race conditions
  ...
  const capacity = barber.seats.filter(s => s.isAvailable && s.status !== 'maintenance').length;
  const currentSameTimeBookings = await Booking.countDocuments({ barberId: barber._id, status: { $in: ["confirmed", "arrived", "in-progress", "pending"] }, startTime: start });
  
  if (currentSameTimeBookings >= capacity) {
      // Over capacity at this exact start time, force queue
  }
  ...
  if (seatIndex !== undefined && !suggestion.addToQueue) {
    ...
    if (seat && seat.isAvailable && seat.status !== 'maintenance') {
...
function formatBooking(b) {
  ...
  return {
    ...
    staffId: b.staffId, paymentStatus: b.paymentStatus
  };
}
...
async function listForBarber(req, res) {
  ...
  const bookings = await Booking.find({ barberId: barber._id }).populate("serviceIds").populate("customerId", "name email phone avatarUrl").sort({ startTime: -1 });
  const out = bookings.map(b => ({
    ...formatBooking(b),
...
async function availableSlots(req, res) {
  ...
  const barber = await Barber.findById(barberId);
  if (!barber || !barber.isShopOpen || barber.pauseBookings) return res.json({ slots: [] });
  ...
  for (let m = openM; m + totalNeeded <= closeM; m += slotMinutes) {
    ...
    const isInBreak = breaks.some(br => {
        const brStart = new Date(br.startTime);
        const brEnd = new Date(br.endTime);
        return (start < brEnd && endWithBuffer > brStart); // overlap
    });
...
path = r"C:\Users\ULTRA\Pictures\Desktop\BARBER\scratch\gen_booking_controller_v2.py"
with open(path, "w", encoding="utf-8") as f:
    f.write(file_content)

print("bookingController.js script generated successfully!")
