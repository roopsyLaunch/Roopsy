...
const BANNER_IMAGE = "https://images.unsplash.com/photo-1503951914875-452162b0f3f1?w=500&auto=format&fit=crop&q=80";

export function BarberListScreen({ navigation }) {
  ...
  const load = useCallback(async () => {
    try {
      ...
      setItems(res.data.barbers || []);
    } catch (err) {
      console.error("BarberListScreen load error:", err);
    }
  }, [gpsCoords]);
  ...
  const filteredBarbers = items.filter(barber => {
    const matchesSearch = barber.shopName?.toLowerCase().includes(searchQuery.toLowerCase()) || 
                          barber.address?.city?.toLowerCase().includes(searchQuery.toLowerCase()) ||
                          barber.address?.line1?.toLowerCase().includes(searchQuery.toLowerCase());
    
    let matchesCategory = true;
    if (selectedCategory === "Open Now") {
      matchesCategory = barber.isShopOpen;
    } else if (selectedCategory === "Top Rated") {
      const ratingVal = parseFloat(barber.averageRating || barber.rating || 4.5);
      ...
      <View style={styles.topRow}>
        ...
        <View style={styles.headerTitleContainer}>
          <View style={styles.iconCircle}>
            <Ionicons name="cut-outline" size={20} color="#2563eb" />
          </View>
          <View>
            <Text style={styles.headerTitle}>Barber Shops</Text>
      ...
      <View style={styles.searchRow}>
        <View style={styles.searchBox}>
          ...
          <TextInput 
            ...
            placeholder="Search barber shops near you..."
      ...
      <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.pillScroll}>
        {CATEGORIES.map(cat => {
          ...
          return (
            ...
            >
              <Ionicons name={cat.icon} size={16} color={isActive ? "#2563eb" : "#64748b"} style={{ marginRight: 6 }} />
  ...
  const renderBanner = () => (
    <View style={styles.bannerContainer}>
      ...
      <LinearGradient
        colors={["rgba(37, 99, 235, 0.85)", "rgba(37, 99, 235, 0.4)"]}
        start={{ x: 0, y: 0 }}
        end={{ x: 1, y: 0 }}
        style={styles.bannerOverlay}
      >
        <View style={styles.bannerTextSection}>
          <Text style={styles.bannerTitle}>Expert Haircuts & Styling</Text>
          <Text style={styles.bannerSubtitle}>Sharp Look. Every Time.</Text>
        </View>
  ...
  const renderSectionTitle = () => (
    <View style={styles.sectionTitleRow}>
      <Text style={styles.sectionTitle}>Top Barber Shops Near You</Text>
      <Pressable>
  ...
  const renderTrustIcons = () => (
    <View style={styles.trustContainer}>
      <View style={styles.trustItem}>
        ...
        <Text style={styles.trustTitle}>Verified Barbers</Text>
        <Text style={styles.trustDesc}>Certified & hygienic barbershops</Text>
      </View>
      <View style={styles.trustItem}>
        <View style={styles.trustIconCircle}>
          <Ionicons name="cut-outline" size={24} color="#2563eb" />
        </View>
        <Text style={styles.trustTitle}>Precision Styling</Text>
        <Text style={styles.trustDesc}>Expert hair & beard artists</Text>
      </View>
      <View style={styles.trustItem}>
        <View style={styles.trustIconCircle}>
          <Ionicons name="time-outline" size={24} color="#2563eb" />
        </View>
        <Text style={styles.trustTitle}>Zero Waiting</Text>
        <Text style={styles.trustDesc}>Priority appointment slots</Text>
      </View>
      <View style={styles.trustItem}>
        <View style={styles.trustIconCircle}>
    ...
    <View style={styles.container}>
      ...
      {loading ? (
        ...
        <FlatList
          ...
          renderItem={({ item }) => {
            const services = item.specialties || ["Haircut", "Beard Trim", "Shave", "Facial"];
            ...
            return (
              <Pressable
                style={styles.card}
                onPress={() => navigation.navigate("BarberDetail", { barberId: bId, shopName: item.shopName })}
              >
                ...
                <View style={styles.cardInfo}>
                  ...
                  <Text style={styles.specialtyText} numberOfLines={1}>
                    {item.businessCategory || "Haircut & Grooming Specialist"}
                  </Text>
          ...
          ListEmptyComponent={
            <View style={styles.emptyContainer}>
              ...
              <Text style={styles.emptyText}>There are no barbershops available in your area matching your criteria.</Text>
            </View>
          }
        />
      )}
...
const styles = StyleSheet.create({
  ...
  iconCircle: { width: 40, height: 40, borderRadius: 20, backgroundColor: "#dbeafe", justifyContent: "center", alignItems: "center", marginRight: 12 },
  ...
  bannerContainer: { marginHorizontal: 20, marginBottom: 24, borderRadius: 16, overflow: "hidden", height: 95 },
  bannerImage: { width: "100%", height: "100%", position: "absolute" },
  bannerOverlay: { position: "absolute", top: 0, left: 0, right: 0, bottom: 0, paddingHorizontal: 16, flexDirection: "row", alignItems: "center", justifyContent: "space-between" },
  bannerTextSection: { flex: 1, marginRight: 12 },
  bannerTitle: { fontSize: 16, fontWeight: "800", color: "#ffffff", marginBottom: 2 },
  bannerSubtitle: { fontSize: 11, fontWeight: "600", color: "rgba(255,255,255,0.9)" },
  bannerBtn: { flexDirection: "row", alignItems: "center", backgroundColor: "#ffffff", paddingHorizontal: 12, paddingVertical: 6, borderRadius: 8 },
  bannerBtnText: { color: "#2563eb", fontSize: 12, fontWeight: "700", marginRight: 4 },
  ...
  serviceTag: { backgroundColor: "#f8fafc", paddingHorizontal: 8, paddingVertical: 4, borderRadius: 6, borderWidth: 1, borderColor: "#f1f5f9" },
  serviceTagText: { fontSize: 10, color: "#2563eb", fontWeight: "600" },
  ...
  bookBtn: { backgroundColor: "#2563eb", paddingHorizontal: 16, paddingVertical: 8, borderRadius: 8 },
  ...
  trustIconCircle: { width: 48, height: 48, borderRadius: 24, backgroundColor: "#ffffff", justifyContent: "center", alignItems: "center", marginBottom: 12, shadowColor: "#2563eb", shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.1, shadowRadius: 8, elevation: 2 },
